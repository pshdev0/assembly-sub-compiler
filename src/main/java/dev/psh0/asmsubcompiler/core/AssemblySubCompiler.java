...
public class AssemblySubCompiler extends Plugin {
    ...
    public static void loadPluginClasses(File pluginClassDir) {
        System.out.println();
        System.out.println("Loading: " + pluginClassDir.getAbsolutePath());
        System.out.println();

        // attempt to load all compiler dev.psh0.asmsubcompiler.core.plugins
        ...
        try {
            ...
            if(urls == null) {
                System.out.println("Error, cannot load plugins");
                return;
            }
            for(var c1 = 0; c1 < urls.length; c1++) {
                System.out.println(urls[c1]);
            }
            cl = new URLClassLoader(urls); // create a new class loader with the directory
        } catch (MalformedURLException ignored) {
        }

        // get the dev.psh0.asmsubcompiler.core.plugins from the directory
        assert cl != null;
        ...
        // load all dev.psh0.asmsubcompiler.core.plugins
        for (String pluginStr : pluginList) {

            if(!pluginStr.endsWith(".class")) continue; // only deal in Java class objects
            if(!pluginStr.startsWith("dev.psh0.asmsubcompiler.plugins.")) pluginStr = "dev.psh0.asmsubcompiler.plugins." + pluginStr; // prepend the package
...
//            log(state.tab + Colors.BLUE_BOLD + "symbols = []");
        ...
        for(Map.Entry<String, Symbol> s : state.symbols.entrySet()) {
            ...
            for(Usage loc : symbol.usages) {
                ...
                removeList.add(loc);
                sortedListOfActiveUnknownSymbolUsageLineNumbers.remove(loc.line);
            }
