...
import dev.psh0.asmsubcompiler.core.Colors;
import dev.psh0.asmsubcompiler.core.CompilerState;
import dev.psh0.asmsubcompiler.huffman.HTree;
import dev.psh0.asmsubcompiler.core.Plugin;
import dev.psh0.asmsubcompiler.huffman.HCode;
   ...
   public boolean functionWithParams(CompilerState state) throws Exception {
      switch (state.command) {
         ...
         case ".vera_decompress_upload" -> {
            ...
            int[] compressedBytes = PluginStandardLibrary.huffmanCompress(loadedData, state.tab); // get the compressed bytes
            ...
            HTree tree = HCode.buildTree(charFreqs); // build tree
            ...
            HCode.buildAsmCode(tree, new StringBuffer(), "", asmString);
