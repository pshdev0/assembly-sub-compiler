...
public class Plugin {
   ...
   protected static void addMnemonic(String details) {
      ...
      while (counter < tokens.length) {
         ...
         if(!addressingModeHashMaps.containsKey(addressingMode)) addressingModeHashMaps.put(addressingMode, new HashMap<>());
   ...
   public static void log(String text1, String text2, String text3) {
      ...
      if(Plugin.sortedListOfActiveUnknownSymbolUsageLineNumbers.size() == 0 || Plugin.sortedListOfActiveUnknownSymbolUsageLineNumbers.first() >= compilerOutput.size()) {
         for(int c1 = Plugin.printedToLine; c1 < compilerOutput.size(); c1++) System.out.println(compilerOutput.get(c1));
   ...
   protected static ArrayList<Integer> add(int pc, String str, Object... values) throws Exception {
      ...
      for(Object x : values) {
         if(x instanceof String) {
            ...
            if(val >= -128) output.append(Integer.toHexString(val)).append(" "); // don't output infill codes (we will add the real values later)
         }
      }
//      if(str != null) log(String.format("%-20s %-50s %s", Integer.toHexString(pc), str, ": " + Color.CYAN + output));
   ...
   public static void addInfillFunction(Integer id, InfillLambda func) throws Exception {
      if(infillFunctions.containsKey(id)) throw new Exception("Duplicate infill function id found, choose a different id.");
   ...
   protected static String loadASMCode(String fileName) {
      File file = new File("/Users/paul/Code/IntelliJ/AssemblySubCompiler/src/asm/" + fileName + ".asm");
      try {
   ...
   protected enum Color {
    ...
    public boolean functionNoParams(CompilerState state) throws Exception { return false; }
    public boolean functionWithParams(CompilerState state) throws Exception { return false; }
    public boolean commandBlocks(CompilerState state) throws Exception { return false; }
}
